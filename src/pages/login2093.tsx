  ...
  const changeWords = (str: string, value: number) => {
    ...
    for(const char of str){
        if('A'.charCodeAt(0)<=char.charCodeAt(0) && char.charCodeAt(0)<='Z'.charCodeAt(0))
            result+=String.fromCharCode(((char.charCodeAt(0)-'A'.charCodeAt(0)+value)%26)+'A'.charCodeAt(0));
        else if('a'.charCodeAt(0)<=char.charCodeAt(0) && char.charCodeAt(0)<='z'.charCodeAt(0))
            result+=String.fromCharCode(((char.charCodeAt(0)-'a'.charCodeAt(0)+value)%26)+'a'.charCodeAt(0));
        else{
            result+=char;
        }
